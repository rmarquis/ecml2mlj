...
The score prediction results on the AFL data sets are shown in Figure~\ref{fig:fig:WLAccuracy_AFL} and Figure~\ref{fig:WLAccuracy_AFL_without_GaussianOD}. Clearly, the Gaussian-OD model fails in predicting the scores of the matches, because the differences in the learnt skill levels between teams are relatively small. A Gaussian likelihood function with the small difference has low probabilty of generating huge match scores for the AFL data set.
...
%%%% results on three data sets with three criteria: 9 figures;
\begin{center}
\begin{figure*}[t!]
 \centering
...
\subsubsection{Multi-Class Classification Evaluated with the Brier Score}

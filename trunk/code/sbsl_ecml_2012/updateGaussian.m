function [o_i d_j] = updateGaussian( offenceGaussian, defenceGaussian, score, beta, gamma )
...
%     end    
    ...
    if noTeams == 2
       ...
       [varBag, MessageBag] = UpdateHelperGaussianWeightedSumFactor(weight0, weight0Squared, m1, m2, m3, MessageBag, ...
                                                                                              v1, v2, v3, varBag);
       % Send the Gaussian factor N(si; x, gamma^2) --> N(x; si, gamma^2)
       % Marginal of Gaussian is the multiplication of two incoming
       % messages
       varXID = 2*noPlayers + 1;
       
%        newMu = score + gamma*vFun(score/gamma, 0);
%        newVariance = gamma^2 *(1 - wFun(score/gamma, 0));
%        varBag{varXID} = varBag{varXID} * Gaussian(newMu, sqrt(newVariance) );   

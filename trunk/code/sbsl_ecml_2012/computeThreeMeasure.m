...
function [resultFinal resultFinalWLD skillPercentageYear scoreAccuracyPercentageYear] = computeThreeMeasure(matchOutcome, trainingPercentage, testPercentage)
skillYear = cell(1, 5);
...
predictedScoreGaussianSD = zeros( numMatch, 2);
...
for idxTestData = 1:numMatch
    ...
    pPoisson = 0;
    for tempCounter = 1:100 % approximation to calculate the winProb. Lower bound
        pPoisson = pPoisson + ...
    ...
    predictedScorePoisson(idxTestData, 1) = abs(lambdaI-matchOutcomeTesting(idxTestData, 3))/matchOutcomeTesting(idxTestData, 3);
    predictedScorePoisson(idxTestData, 2) = abs(lambdaJ-matchOutcomeTesting(idxTestData, 4))/matchOutcomeTesting(idxTestData, 4);
    if matchOutcomeTesting(idxTestData, 3) == 0
        flagZeroTeamI(idxTestData) = 0;
    end
    if matchOutcomeTesting(idxTestData, 4) == 0
        flagZeroTeamJ(idxTestData) = 0;
    end
    ...
    predictedScoreGaussian(idxTestData, 1) = abs(mu1 - matchOutcomeTesting(idxTestData, 3))/matchOutcomeTesting(idxTestData, 3);
    predictedScoreGaussian(idxTestData, 2) = abs(mu2 - matchOutcomeTesting(idxTestData, 4))/matchOutcomeTesting(idxTestData, 4);
    ...
    dif = matchOutcomeTesting(idxTestData, 3) - matchOutcomeTesting(idxTestData, 4);
    predictedScoreGaussianSD(idxTestData) = abs( mu - dif )/abs(dif);
    if dif ==0
    ...
    pLogistic = 1/(1+exp(-tempResLogistic(2)));
...
pos = find(flagZero == 0);
predictedScoreGaussianSD(pos) = [];

posI = find(flagZeroTeamI == 0);
posJ = find(flagZeroTeamJ == 0);
allZeroIJ = unique([posI posJ]);
predictedScorePoisson(allZeroIJ, :) = [];
predictedScoreGaussian(allZeroIJ, :) = [];
scoreAccuracyPercentageYear = [mean(predictedScorePoisson(:)) mean(predictedScoreGaussian(:)) mean(predictedScoreGaussianSD); ...
    std(predictedScorePoisson(:))/sqrt(2*size(predictedScorePoisson,1)) std(predictedScoreGaussian(:))/sqrt(2*size(predictedScoreGaussian,1)) std(predictedScoreGaussianSD)/sqrt(length(predictedScoreGaussianSD))];
end